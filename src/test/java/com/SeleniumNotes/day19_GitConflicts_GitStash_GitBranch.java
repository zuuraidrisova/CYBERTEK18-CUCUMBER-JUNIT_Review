...
Q: When I will go to work, am I gonna create github repo?
  ...
  If it's a brand new project, there is gonna be some admin, who creates accounts,
   repositories, etc..

Q: How to clone and what to clone?

  - you are making clone to get a copy of the repo. Everyone works on their own copyof repo.
...
remote repository - repository on the server (github), browser
...
this command should be executed only once, to initialize git repository on ur local
 computer. Make sure that you run this command in the project folder.
...
First thing that you have to do is to create a .gitignore file, we create this file
to tell git which files do not have to be tracked and commited
...
ex: git commit -m "@VIT-1231 User should be able to edit event's end time"

use git log ==>  to see the history of commits. You will se smth as below:
...
NOTE: by default, you might not have markdown plugin, so please install
    it whenever intellij is offering.
...
git commit -a -m "commit message"  is the same as git add . and  git commit

git fetch - to pull updates from remote to local repo
...
Step 2.Go over conflicting files. If you are not sure what should stay in the final
 version of conflicting files, you can text/call to the person that can help you
 to decide what should stay.
...
To apply first stash/to get stashed files back

    git stash pop

Then, you need to resolve conflicts. After you resolved all conflicts, you can commit
    and push your code.

Why do we need to stash?

	Let's say we need to do the pull, and we have some unfinished work.
...
usually, master branch is locked from direct commits. Instead, everyone is working
 on their own branch.
you are working within your own branch, commit and push to the remote,
but still your branch.
...
How to push new branch to github/how to create a new branch for ur new local repo?
