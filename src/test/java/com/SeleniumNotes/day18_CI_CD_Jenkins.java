...
public class day18_CI_CD_Jenkins {
    /*
    CI/CD  and  Jenkins server - computer that is used to serve other computers. Environment - place where server
     is running application. All environments, except production, have a dummy data (fake data). dev --> test -->
     stage --> production  dev - used by developers only test - used for functional testing, used by us and for
     non-functional testing like performance. stage - UAT (User Acceptance Testing) production -
     the real application, used by end-users. NOTE: There could be other environments, not every
     single company has only these 4. For example: UAT, Integration, pre-prod, TESTA, TESTB, TESTC,
      etc. when new build is deployed to any environment, smoke test should be automatically executed.
       ...
       could broke something), and periodically (every morning), because something could happened
        overnight that caused server problems. After release, we do smoke test in the production
         environment, fore sure.
...
use VCS(version control system, like GIT) and integrate frequently their changes.
 Also, those changes will be automatically tested. (code quality scans, unit and integration tests)
  ...
  I would say it's easier to digest small pieces little by little rather than digesting
   huge bulk at once. CD -Continuous Delivery Continuous Delivery is an automated build
   and execution of at unit and integration tests, performing code analysis, functional
...
goes through the pipeline, and if it passes all the tests, it automatically gets deployed
 ...
 of manual steps. Continues Integration - build + unit test + integration tests + code analysis
 Continues Delivery - Continues Integration + smoke testing + regression testing +
  ...
  except release. Continues Deployment - Continues Delivery + automatic release.
   The goal of CI/CD is a shorter release cycle. So end-users can get new version
   of application, with new features of with fixed bugs, faster. Faster software release -->
    better software --> higher client satisfaction

 Q: Is it possible to release new software without testing? -
 A: No, that's why testing/QA plays essential role in this entire process. Before releasing a new version of software,
   regression testing MUST be done. Regression testing can be done either manually
   or automatically. Manually - takes time, less accurate. If regression testing is done manually, forget about short release cycles. In my company, before automation, regression testing was done manually, and it was about 2 weeks. Before I left, our regression was running around 2.5 hours. It's because of parallelization and running 35 browsers at once. We used 5 servers to distribute tests and execute them faster. We were running regression in stage environment. every company have to have at least 4 environments: dev --> qa --> stage --> production

#dev - reserved for developers, once changes were committed, devs can build a new version
of software and deploy it to dev environment. #qa - reserved for us, this is were we
 do testing #stage - preparation for release, this is were performance testing
...
deployment - process of creating a new build and delivering it to the application server.
 application server - program that makes web application running and accessible over the browser.
  ...
  Newest version in dev, oldest in production. Every environment has its own database,
   ...
   release manually. Create AWS account: https://aws.amazon.com/console/ ROLE:
...
and innovate faster. Jenkins Jenkins  is  an  open-source  automation  tool  written
...
With Jenkins, organizations can accelerate the software development process  through  automation.
  Jenkins  integrates  development  life-cycle processes of all kinds, including build,
   document, test, package, stage, deploy, static analysis, and much more.
    Jenkins  achieves  Continuous  Integration  with  the  help  of  plugins.
     Plugins allows the integration of Various DevOps stages. If you want to
      integrate a particular tool, you need to install the plugins for that tool.
        ...
        default port for Jenkins is 8080. In our case, it was switched to 8081.
         ...
         3. Build -->> Invoke Top-level maven targets --> clean test -P Smoke or, if you don't have a profile, use: clean test -Dcucumber.options="--tags @smoke_test"
         ...
         5. Click build now!!! Regarding scheduling tests: This field follows the syntax of cron
           (with minor differences). Specifically, each line consists of 5 fields separated by
            TAB or whitespace: MINUTE HOUR DOM MONTH DOW MINUTE   Minutes within the hour (0–59)
             HOUR     The hour of the day (0–23) DOM The day of the month (1–31) MONTH
               The month (1–12) DOW The day of the week (0–7) where 0 and 7 are Sunday.
  ...
  around 6 AM MAKE SURE TO PUT SPACESTZ=US/Eastern H 6 * * 1-5 H 21 * * *
     */
